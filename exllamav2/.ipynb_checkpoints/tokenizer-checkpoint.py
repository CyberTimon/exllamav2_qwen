...
class ExLlamaV2Tokenizer:
    ...
    def __init__(self, config, lazy_init = False, force_json = False):
        ...
        path_hf_auto = os.path.join(self.config.model_dir, "vocab.json") # Fallback for Qwen
        ...
        elif os.path.exists(path_hf_auto): self.tokenizer = ExLlamaV2TokenizerAutoHF(self.config.model_dir)
        ...
        special_tokens_map_path = os.path.join(self.config.model_dir, "special_tokens_map.json")
        if os.path.exists(special_tokens_map_path):
            with open(special_tokens_map_path, encoding="utf8") as f:
                special_tokens_map = json.load(f)
                additional_special_tokens = special_tokens_map.get("additional_special_tokens", [])
                for token in additional_special_tokens:
                    # Assuming all additional special tokens are indeed special
                    self.extended_piece_to_id[token] = len(self.extended_piece_to_id)
